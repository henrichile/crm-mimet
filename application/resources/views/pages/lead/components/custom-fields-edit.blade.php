...
<div class="form-group row m-b-8">
    ...
    @endif    
    <div class="col-sm-12">
                @if($field->customfields_name=='lead_custom_field_3')
            <select name="{{ $field->customfields_name }}" id="{{ $field->customfields_name }}" class="form-control form-control-sm" onchange="javascript:sexo(this.value)">
                 <option value="" @if(customFieldValue($field->customfields_name, $lead, 'form')=='') selected="selected" @endif>Seleccione</option>
                <option value="Masculino" @if(customFieldValue($field->customfields_name, $lead, 'form')=='Masculino') selected="selected" @endif>Masculino</option>
                <option value="Femenino" @if(customFieldValue($field->customfields_name, $lead, 'form')=='Femenino') selected="selected" @endif>Femenino</option>
            </select>
            
            
            
        @elseif($field->customfields_name=='lead_custom_field_2')
            <select name="{{ $field->customfields_name }}" id="{{ $field->customfields_name }}" class="form-control form-control-sm" onchange="javascript:traeCursos(this.value)">
                 <option value="" @if(customFieldValue($field->customfields_name, $lead, 'form')=='') selected="selected" @endif>Seleccione</option>
                <option value="Santiago Centro" @if(customFieldValue($field->customfields_name, $lead, 'form')=='Santiago Centro') selected="selected" @endif>Santiago Centro</option>
                <option value="Vitacura" @if(customFieldValue($field->customfields_name, $lead, 'form')=='Vitacura') selected="selected" @endif>Vitacura</option>
                <option value="La Cisterna" @if(customFieldValue($field->customfields_name, $lead, 'form')=='La Cisterna') selected="selected" @endif>La Cisterna</option>
                <option value="Curacavi" @if(customFieldValue($field->customfields_name, $lead, 'form')=='Curacavi') selected="selected" @endif>Curacaví</option>
                <option value="Intensivo" @if(customFieldValue($field->customfields_name, $lead, 'form')=='Intensivo') selected="selected" @endif>Intensivo</option>
                <option value="La Reina" @if(customFieldValue($field->customfields_name, $lead, 'form')=='La Reina') selected="selected" @endif>La Reina</option>
                
            </select>    
        @elseif($field->customfields_name=='lead_custom_field_1')
        <div id="mosprod">
             <input type="hidden" name="cursox" id="cursox" value="{{ customFieldValue($field->customfields_name, $lead, 'form') }}">
            <select name="{{ $field->customfields_name }}" id="{{ $field->customfields_name }}" class="form-control form-control-sm">
                 <option value="" @if(customFieldValue($field->customfields_name, $lead, 'form')=='') selected="selected" @endif>Seleccione</option>
            </select> 
        </div>
        @elseif($field->customfields_name=='lead_custom_field_9')
            <select name="{{ $field->customfields_name }}" id="{{ $field->customfields_name }}" class="form-control form-control-sm">
                 <option value="" @if(customFieldValue($field->customfields_name, $lead, 'form')=='') selected="selected" @endif>Seleccione</option>
                <option value="si" @if(customFieldValue($field->customfields_name, $lead, 'form')=='si') selected="selected" @endif>Si</option>
                <option value="no" @if(customFieldValue($field->customfields_name, $lead, 'form')=='no') selected="selected" @endif>No</option>
            </select>
        
         @elseif($field->customfields_name=='lead_custom_field_6')
           @php
      ...
      $nacionalidad=json_decode($nacionalidades);
           ...
           @endphp
            ...
            <select name="{{ $field->customfields_name }}" id="{{ $field->customfields_name }}" class="form-control form-control-sm">
                ...
                <option value="{{ $item->Nombre }}" @if(customFieldValue($field->customfields_name, $lead, 'form')==$item->Nombre) selected="selected" @endif>{{ $item->Nombre }}</option>
                @endforeach
            </select>
        @elseif($field->customfields_name=='lead_custom_field_11' || $field->customfields_name=='lead_custom_field_12')
           
        @else
        <input type="text" class="form-control form-control-sm @if($field->customfields_name=='lead_custom_field_4')  card-pickadate @endif" id="{{ $field->customfields_name }}"
     ...
     <div id="mostemb" @if(customFieldValue('lead_custom_field_3', $lead, 'form')=='no') style="display:none !important" @endif>
          <label
            class="col-sm-12 text-left control-label col-form-label font-13 p-b-1 {{ runtimeCustomFieldsRequiredCSS($field->customfields_required) }}">
              ¿Esta embarazada?
          </label>
         <div class="col-sm-12">
             <select name="lead_custom_field_11" id="lead_custom_field_11" class="form-control form-control-sm">
                     <option value="" @if(customFieldValue('lead_custom_field_11', $lead, 'form')=='') selected="selected" @endif>Seleccione</option>
                    <option value="si" @if(customFieldValue('lead_custom_field_11', $lead, 'form')=="si") selected="selected" @endif>Si</option>
                    <option value="no" @if(customFieldValue('lead_custom_field_11', $lead, 'form')=="no") selected="selected" @endif>No</option>
                </select>
         </div>   
         <label
            class="col-sm-12 text-left control-label col-form-label font-13 p-b-1 {{ runtimeCustomFieldsRequiredCSS($field->customfields_required) }}">
         ...
         <div class="col-sm-12">
             <select name="lead_custom_field_12" id="lead_custom_field_12" class="form-control form-control-sm">
                     <option value="" @if(customFieldValue('lead_custom_field_12', $lead, 'form')=='') selected="selected" @endif>Seleccione</option>
                     
                    @for($i=1;$i<=9;$i++) 
                    <option value="{{ $i }}" @if(customFieldValue('lead_custom_field_12', $lead, 'form')==$i) selected="selected" @endif>{{ $i }}</option>
                    @endfor
                </select>
         </div>  
     </div>
